...
function onepage_checkout_for_edd_ajax_cart_total_qty( $return ) {
	$return['total_qty'] = sanitize_text_field( EDD()->cart->get_quantity() );
	return $return;
}
add_filter( 'edd_ajax_cart_item_quantity_response', 'onepage_checkout_for_edd_ajax_cart_total_qty' );

/**
 * Get the URL of the Checkout page
 * Modification of EDD Checkout URL using filter
 */
function onepage_checkout_for_edd_get_checkout_uri( $uri ) {
	$uri = false;
	...
	return apply_filters( 'onepage_checkout_for_edd_get_checkout_uri', esc_url( $uri ) );
}
add_filter( 'edd_get_checkout_uri', 'onepage_checkout_for_edd_get_checkout_uri', 15 );

/**
 * Modal Content Ajax
 */
function onepage_checkout_for_edd_update_checkout_ajax(){
	// nonce check
	check_ajax_referer( 'eddnstant_nonce', 'nonce' );
...
add_action( 'wp_ajax_eddnstant_update_checkout', 'onepage_checkout_for_edd_update_checkout_ajax' );
add_action( 'wp_ajax_nopriv_eddnstant_update_checkout', 'onepage_checkout_for_edd_update_checkout_ajax' );
 ...
 * @return  string
 */
if ( ! function_exists('onepage_checkout_for_edd_get_svg_icon') ) {
	function onepage_checkout_for_edd_get_svg_icon( $icon = null ){
 ...
 * @return  void
 */
if ( ! function_exists('onepage_checkout_for_edd_svg_icon') ) {
	function onepage_checkout_for_edd_svg_icon( $icon = null ){
		echo onepage_checkout_for_edd_get_svg_icon( $icon );
	}
}
 ...
 */
function onepage_checkout_for_edd_is_checkout() {
	return true;
}
add_filter( 'edd_is_checkout', 'onepage_checkout_for_edd_is_checkout', 15 );

/**
 * Cart Count function
 */
if ( ! function_exists( 'onepage_checkout_for_edd_cart_count' ) ) {
	function onepage_checkout_for_edd_cart_count() {
		if ( ! function_exists( 'edd_get_cart_quantity' ) ) {
			return;
		}
		?>
		<span class="eddnstant_cart_total"><?php echo esc_html( edd_get_cart_quantity() ); ?></span> 
		<?php
	}
}
 ...
 *	Custom CSS function
 */
if( !function_exists( 'onepage_checkout_for_edd_custom_css' ) ){
	function onepage_checkout_for_edd_custom_css(){
		...
		// Panel Width
		if( !empty( $get_options['panel-width'] ) ) :
			$output .= '
			@media (min-width: 768px) {
				.eddnstant-checkout-wrap {
					width: ' . sanitize_text_field( $opcfedd_opt['panel-width'] ) . '%;
					min-width: ' . sanitize_text_field( $opcfedd_opt['panel-min-width'] ) . 'px;
					height: ' . sanitize_text_field( $opcfedd_opt['panel-height'] ) . '%;
				}
			}
			';
		endif;
		...
		// Modal BG Color
		if( !empty( $opcfedd_opt['panel-bg'] ) ) :
			$output .= '
			.eddnstant-checkout-wrap {
				background: ' . sanitize_text_field( $opcfedd_opt['panel-bg'] ) . ';
			}
			';
		endif;

		// Sticky cart BG Color
		if( !empty( $opcfedd_opt['sticky-cart-bg'] ) ) :
			$output .= '
			.eddnstant-sticky-cart {
				background: ' . sanitize_text_field( $opcfedd_opt['sticky-cart-bg'] ) . ';
			}
			';
		endif;

		// Cart text color
		if( !empty( $opcfedd_opt['sticky-cart-color'] ) ) :
			$output .= '
			.eddnstant-sticky-cart .eddnstant_cart_total {
				color: ' . sanitize_text_field( $opcfedd_opt['sticky-cart-color'] ) . ';
			}
			';
		endif;

		// Cart Count BG Color
		if( !empty( $opcfedd_opt['sticky-cart-count-bg'] ) ) :
			$output .= '
			.eddnstant-sticky-cart .eddnstant_cart_total {
				background: ' . sanitize_text_field( $opcfedd_opt['sticky-cart-count-bg'] ) . ';
			}
			';
		endif;

		// Icon color
		if( !empty( $opcfedd_opt['sticky-cart-icon-color'] ) ) :
			$output .= '
			.eddnstant-sticky-cart svg {
				fill: ' . sanitize_text_field( $opcfedd_opt['sticky-cart-icon-color'] ) . ';
			}
			';
		endif;

		// Z-index
		if( !empty( $opcfedd_opt['panel-zindex'] ) ) :
			$output .= '
			.eddnstant-checkout-wrap {
				z-index: ' . sanitize_text_field( $opcfedd_opt['panel-zindex'] ) . ';
			}
			';
		endif;

		// Custom CSS
		$output .= isset( $get_options['custom-css'] ) && !empty( $get_options['custom-css'] ) ? $get_options['custom-css'] : ""; //Custom css

		// output as styles
		wp_add_inline_style( 'onepage-checkout-for-edd', $output );
	}
}
add_action( 'wp_enqueue_scripts', 'onepage_checkout_for_edd_custom_css', 200 );

// Field Name Function
function opcfedd_field_name( $field_name = '' ){
	return 'opcfedd_opt[' . $field_name . ']';
}

// Get Options Value
function opcfedd_get_option( $option_name = '' ){
	
	$opcfedd_opt = get_option( 'opcfedd_opt', array(
		'cart-position' => '0',
		'show-close-btn' => '1',
		'panel-width' => '40',
		'panel-min-width' => '600',
		...
		'sticky-cart-bg' => '',
		'sticky-cart-color' => '',
		'sticky-cart-count-bg' => '',
		'sticky-cart-icon-color' => '',
		'panel-zindex' => '99999',
		'custom-css' => '',
	) );

	if( !empty( $option_name ) ){
		return isset( $opcfedd_opt[$option_name] ) ? $opcfedd_opt[$option_name] : null;
	}
...
// Options Save Ajax
add_action( 'wp_ajax_one-page-edd-options', 'opcfedd_save_options' );
function opcfedd_save_options(){

	// Check Nonce
	check_ajax_referer( 'one-page-edd-options-nonce', '_wpnonce' );

	// Get Options
	$opcfedd_opt = opcfedd_get_option();

	// Get New Options
	$new_options = isset( $_POST['opcfedd_opt'] ) ? $_POST['opcfedd_opt'] : array();

	// Merge Options
	$opcfedd_opt = array_merge( $opcfedd_opt, $new_options );

	// Update Options
	update_option( 'opcfedd_opt', $opcfedd_opt );

	// Return
	wp_send_json_success( array(
		'message' => esc_html__( 'Options Saved Successfully', 'onepage-checkout-for-edd' ),
	) );
}

// CodeMirror JS Enqueue
function opcfedd_code_editor_enqueue_scripts() {
	...
	wp_add_inline_script(
		'code-editor',
		sprintf( 'jQuery( function() { wp.codeEditor.initialize( "custom-css", %s ); } );', wp_json_encode( $settings ) )
	);

}
add_action( 'admin_enqueue_scripts', 'opcfedd_code_editor_enqueue_scripts' );