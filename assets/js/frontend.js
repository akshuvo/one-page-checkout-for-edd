(function($) {
	...
	$(document).ready(function(){
		...
		$(document.body).on('edd_cart_item_added', function( e, response ){
            ...
            $.ajax({
                type: 'POST',
                url: opcfedd_ajax_vars.ajaxurl,
                data: {
                    'action': 'eddnstant_update_checkout',
                    'nonce': opcfedd_ajax_vars.nonce,
                    'response': response,
                },
		...
		// Panel Toggle
		$(document).on('click', '#eddnstant-toggler, #eddnstant-sticky-cart, .eddnstant-close, a.header-cart', function( e ){
			e.preventDefault();
